...
if dev_mode:
    ...
    uvicorn.run(
        "bridgewell_gpt.main:app",  # Import string instead of direct reference
        host="0.0.0.0", 
        port=settings().server.port, 
        log_config=None,
        reload=True,
        reload_dirs=["bridgewell_gpt"]  # Watch the bridgewell_gpt directory for changes
    )
else:
    # In production mode, import the app directly (no hot reload)
    from bridgewell_gpt.main import app
